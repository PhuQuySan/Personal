...
class CacheManager {
    ...
    constructor() {
        ...
        if (typeof window !== 'undefined') {
            try {
                const stored = localStorage.getItem('global-app-cache');
                if (stored) {
    ...
    private persist(): void {
        if (typeof window !== 'undefined') {
            try {
                ...
                localStorage.setItem('global-app-cache', JSON.stringify(obj));
            } catch (e) {
                // Silently handle storage full errors
            }
        }
    }
    ...
    clear(): void {
        ...
        if (typeof window !== 'undefined') {
            localStorage.removeItem('global-app-cache');
        }
...
export function useCachedData<T>(
    ...
    const updateState = useCallback((newData: T | null, isStaleData: boolean = false) => {
        if (!isMountedRef.current) return;

        setData(prevData => {
            // Instant equality check for objects
            if (JSON.stringify(prevData) === JSON.stringify(newData)) {
                // Still update isStale if it changed
                setIsStale(prev => {
                    if (prev !== isStaleData) return isStaleData;
                    return prev;
                });
                return prevData;
            }
    ...
    const fetchFreshData = useCallback(async (silent: boolean = false) => {
        ...
        try {
            ...
            // Ignored if abort error (standard lifecycle)
            ...
            if (!forceRefresh) {
                ...
                if (pending) {
                    try {
                        ...
                        // In React Strict Mode, the first mount might initiate a pending request and then unmount,
                        // causing the request to abort. The second mount awaits this pending request, which throws an AbortError.
                        // We must catch this AbortError and fall through to try fetching again instead of dying!
                        if (err instanceof Error && err.name === 'AbortError') {
                            console.warn(`[Cache] Pending request for ${key} was aborted, fetching fresh data...`);
                        } else {
                            throw err;
                        }
                    }
...
export function useCachedUserProfile() {
    const { data, loading, error, isStale, refetch } = useCachedData(
        'user-profile',
        async () => {
            const { fetchUserProfile } = await import('@/shared/lib/fetchUserProfile');
